...
class RungeKutta3(Base1):
    ...
    def proxItem(self, yi, x):
        ...
        # Passo 3: Calcule k3
        k3 = self.h * self.resolverFunc(x + self.h, yi - k1 + 2 * k2)
        
        # Passo 4: Calcule o próximo valor de y usando a combinação ponderada de k1, k2 e k3
        result = yi + (1/6) * k1 + (2/3) * k2 + (1/6) * k3
...
def main():
    ...
    while index + 1 < len(linhas):
        ...
        obj = RungeKutta3(func, h, pontoX, maxX, dir_path)
